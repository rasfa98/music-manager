...
module.exports.editAlbum = (db, id, album, tracks, producers) => {
  return new Promise((resolve, reject) => {
    ...
    db.serialize(() => {
      ...
      db.run(
        `DELETE FROM Producers WHERE producerId IN (SELECT producerId FROM ProducedBy WHERE albumId = ?)`,
        id,
        (db, err) => {
          if (err) {
            db.run('ROLLBACK');
      ...
      db.run(
        `DELETE FROM Tracks WHERE trackId IN (SELECT trackId FROM MadeOf WHERE albumId = ?)`,
        id,
        (db, err) => {
          if (err) {
            db.run('ROLLBACK');
